...
def test_get_function_code():
    node = cst.parse_statement("def test_func(x, y): return x + y")
    assert get_function_code(node) == "def test_func(x, y): return x + y"
...
def test_add_import():
    ...
    assert "def test(): pass" in updated_module.code

    # Test adding duplicate import
    updated_module = add_import(updated_module, "import os")
    assert updated_module.code.count("import os") == 1